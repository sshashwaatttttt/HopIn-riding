...
import { getRides, getPreviousRides, subscribeToSync, getBlockedUserIds } from '../utils/store';
...
export const LiveRideBoard = () => {
  ...
  const refreshRides = () => {
    const data = getRides();
    const prevData = getPreviousRides();
    const blocked = getBlockedUserIds() || [];
    const cleanActive = (Array.isArray(data) ? data : []).filter(r => r && (!r.host || !blocked.includes(r.host.id)));
    const cleanPrev = (Array.isArray(prevData) ? prevData : []).filter(r => r && (!r.host || !blocked.includes(r.host.id)));
    setRides(cleanActive);
    setPreviousRides(cleanPrev);
  };
  ...
  useEffect(() => {
    ...
    // Interval check every 15s to transition completed rides and auto-purge expired rides
    const interval = setInterval(refreshRides, 15000);
    return () => {
      unsubscribe();
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="aesthetic-card rounded-3xl p-5 md:p-6 shadow-xl space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {/* Active vs Previous Switcher Tabs */}
            <div className="flex items-center p-1 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
              ...
              >
                ...
                <span>Active</span>
                <span className="text-[10px] opacity-80">({filteredRides.length})</span>
              </button>
              ...
              >
                ...
                <span>Previous</span>
                <span className="text-[10px] opacity-80">({previousRides.length})</span>
              </button>
            </div>
            ...
            <Link
              ...
              className="min-h-[44px] px-3 sm:px-4 py-2 text-xs font-black rounded-2xl bg-gradient-to-r from-amber-500 to-yellow-400 text-slate-950 shadow-md hover:brightness-105 flex items-center gap-1.5 transition-transform active:scale-95"
            >
              <PlusCircle className="w-4 h-4" />
              <span className="hidden sm:inline">Create Ride Slot</span>
              <span className="sm:hidden">Post Slot</span>
            </Link>
          </div>
        </div>
      ...
      {/* Ride Cards Grid: Active vs Previous Tab Display */}
      {activeTab === 'previous' ? (
        <div className="space-y-4 animate-in fade-in duration-200">
          {/* Previous Rides Context Banner */}
          <div className="p-4 rounded-3xl bg-amber-500/10 border border-amber-500/25 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <span className="font-extrabold text-slate-900 dark:text-white block">
                  Previous Rides (Completed within the past hour)
                </span>
                <span className="text-slate-600 dark:text-slate-400 text-[11px]">
                  All co-riders, route details, and fare splits remain visible for 1 hour, after which rides are permanently deleted.
                </span>
              </div>
            </div>
            <span className="px-3 py-1 rounded-xl bg-amber-500/20 text-amber-700 dark:text-amber-300 font-extrabold text-[11px] self-start sm:self-auto shrink-0">
          ...
          {previousRides.length === 0 ? (
            <div className="bg-white dark:bg-gray-900 rounded-3xl p-10 text-center border border-gray-200 dark:border-gray-800 shadow-xl my-6 animate-in fade-in duration-300">
              <div className="w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-400 text-2xl flex items-center justify-center mx-auto mb-3">
                🕒
              </div>
              <h3 className="text-lg font-black text-gray-900 dark:text-white">
                No previous rides in the last hour
              </h3>
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mt-1 max-w-sm mx-auto">
                Completed rides from their departure time appear here for 60 minutes with full details before automatic deletion.
              </p>
            </div>
          ) : (
        ...
        /* Active Rides Grid & Contextual Empty States */
        filteredRides.length === 0 ? (
          <div className="bg-white dark:bg-gray-900 rounded-3xl p-10 text-center border border-gray-200 dark:border-gray-800 shadow-xl my-6 animate-in fade-in duration-300">
            ...
            <h3 className="text-lg font-black text-gray-900 dark:text-white">
              {radiusFilterActive ? 'No ride slots within 2 km of your location' : 'No active ride slots found'}
            </h3>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mt-1 max-w-sm mx-auto">
              {radiusFilterActive
