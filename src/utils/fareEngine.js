...
export const getPlatformDeepLinks = (pickup, dropoff) => {
  ...
  // 3. Rapido Bike & Auto: Booking host with drop=lat,lng coordinate pair and enriched destination keys
  const rapidoQuery = `drop=${dLat},${dLng}&drop_lat=${dLat}&drop_lng=${dLng}&drop_location=${encodeURIComponent(dName)}&drop_address=${encodeURIComponent(dName)}&drop_name=${encodeURIComponent(dName)}&destination=${encodeURIComponent(dName)}&destination_name=${encodeURIComponent(dName)}&d_lat=${dLat}&d_lng=${dLng}&pickup=${pLat},${pLng}&pickup_location=${encodeURIComponent(pName)}&pickup_address=${encodeURIComponent(pName)}&pickup_name=${encodeURIComponent(pName)}&pickup_lat=${pLat}&pickup_lng=${pLng}&p_lat=${pLat}&p_lng=${pLng}&lat=${pLat}&lng=${pLng}`;
  const rapidoWebUrl = `https://www.rapido.bike/`;
  const rapidoAppUrl = `rapido://book?${rapidoQuery}`;
  const rapidoAndroidIntent = `intent://book?${rapidoQuery}#Intent;scheme=rapido;package=com.rapido.passenger;S.drop=${dLat},${dLng};S.drop_location=${encodeURIComponent(dName)};S.drop_address=${encodeURIComponent(dName)};S.destination=${encodeURIComponent(dName)};S.drop_name=${encodeURIComponent(dName)};S.drop_lat=${dLat};S.drop_lng=${dLng};S.pickup=${pLat},${pLng};S.pickup_location=${encodeURIComponent(pName)};S.pickup_address=${encodeURIComponent(pName)};S.pickup_lat=${pLat};S.pickup_lng=${pLng};S.browser_fallback_url=${encodeURIComponent(rapidoWebUrl)};end`;
